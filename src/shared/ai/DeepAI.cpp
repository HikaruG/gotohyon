...
int distance_position(int x1, int x2, int y1, int y2){
...
vector<int> disadvantage(State& state){
    ...
    for(int i : advantage){ // fait en sorte de seulement garder les valeurs négatives
        if(i > 0)
            i =0;
    }
...
bool DeepAI::run(engine::Engine &engine, state::State &state) {
    ...
    my_advantage = disadvantage(state);
    ...
    for(Building * closest_town : towns){
        ...
        if(town_distance > distance_position(ennemy_x, my_x, ennemy_y, my_y)) {
            town_playerid = closest_town->getPlayerId();
            town_x = ennemy_x;
            town_y = ennemy_y;
            town_distance = distance_position(town_x, my_x, town_y, my_y);
        }
    }
    ...
    for(int i= 0; i < (int)my_list_unit.size(); i++) {
        ...
        if (unit_i->getProperty()->getAvailability()) {
            ...
            engine.addCommands(movement_unit);
                ...
                if(distance_position(new_x, town_x, new_y, town_y) < distance_position(old_x, town_x, old_y, town_y))
